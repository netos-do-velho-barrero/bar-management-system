...
public class ProvaController(
    ...
    IMapper mapeador,
    GeradorPdfProva geradorPdfProva
) : Controller
{
    ...
    public ActionResult Duplicar(DuplicarProvaViewModel viewModel)
    {
        ...
        Result resultado = servicoProva
            .Duplicar(new DuplicarProvaDto(viewModel.Id, viewModel.Titulo));
    ...
    {
        return servicoProva.SelecionarDisciplinas()
            .SingleOrDefault(d => d.Id == id)?.Nome;
    }
    ...
    private Result<ConfirmarProvaViewModel> MontarPrevia(EstadoGeracao estado)
    {
        Result<List<QuestaoProvaDto>> resultadoQuestoes = servicoProva
            .SelecionarQuestoes(estado.QuestaoIds);
    ...
    private ActionResult GerarPdf(Guid id, bool incluirGabarito, string prefixoArquivo)
    {
        ...
        byte[] pdf = geradorPdfProva.Gerar(resultado.Value, incluirGabarito);
    ...
    private static string NormalizarNomeArquivo(string titulo)
    {
        ...
        string semAcentos = new(normalizado.Where(caracter =>
            CharUnicodeInfo.GetUnicodeCategory(caracter) != UnicodeCategory.NonSpacingMark).ToArray());
