...
using GeradorDeProvas.Testes.E2E.Modulos.ModuloMateria;
using GeradorDeProvas.Testes.E2E.Modulos.ModuloQuestao;
using Microsoft.Playwright;

namespace GeradorDeProvas.Testes.E2E.Modulos.ModuloProva;
...
public sealed class ProvaE2ETests : E2ETestsBase
{
    ...
    public async Task DeveExibir_ListagemVazia_ParaUsuario_SemProvas()
    {
        ...
        ProvaListarPage listarPage = new(Page, UrlBase);
    ...
    {
        // Arrange
        ProvaListarPage listarPage = await GerarProvaAsync(
            "prova.geracao@teste.local",
            "Prova de Matemática"
        );
    ...
    public async Task DeveBaixar_PdfE_Gabarito_DeUmaProvaGerada()
    {
        ...
        Assert.IsTrue(
            pdf.SuggestedFilename.StartsWith("prova-prova-de-matematica-", StringComparison.Ordinal)
        );
        Assert.IsTrue(pdf.SuggestedFilename.EndsWith(".pdf", StringComparison.Ordinal));

        Assert.IsNull(await gabarito.FailureAsync());
        Assert.IsTrue(
            gabarito.SuggestedFilename.StartsWith("gabarito-prova-de-matematica-", StringComparison.Ordinal)
        );
    ...
    private async Task<ProvaListarPage> GerarProvaAsync(string email, string titulo)
    {
        ...
        ProvaFormPage formPage = new(Page, UrlBase);
        ProvaSelecionarQuestoesPage selecionarQuestoesPage = new(Page, UrlBase);
        ProvaConfirmarPage confirmarPage = new(Page, UrlBase);
        ProvaListarPage listarPage = new(Page, UrlBase);
    ...
    {
        QuestaoFormPage formPage = new(Page, UrlBase);
        ...
        QuestaoListarPage listarPage = new(Page, UrlBase);
