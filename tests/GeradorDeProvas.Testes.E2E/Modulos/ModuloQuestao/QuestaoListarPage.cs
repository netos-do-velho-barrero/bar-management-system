...
namespace GeradorDeProvas.Testes.E2E.Modulos.ModuloQuestao;

public sealed class QuestaoListarPage(
    IPage page,
    string urlBase
)
{
    ...
    public ILocator EnunciadoDaQuestao(string enunciado) => page.GetByRole(
        AriaRole.Heading,
        new() { Name = enunciado, Exact = true }
    );
    ...
    {
        await CardPorEnunciado(nome).GetByRole(
            AriaRole.Link,
    ...
    {
        ILocator enunciadoQuestao = EnunciadoDaQuestao(enunciado);

        return page.Locator(".card").Filter(new() { Has = enunciadoQuestao });
    }
}
