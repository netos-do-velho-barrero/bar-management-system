...
public sealed class QuestaoFormPage(
    IPage page,
    string urlBase
)
{
    ...
    )
    {
        ...
        while (contagemAlternativas < alternativas.Length)
        {
            await page.GetByRole(
                AriaRole.Button,
                new() { Name = "Adicionar alternativa", Exact = true }
            ).ClickAsync();
        }
        ...
        {
            await page.Locator(
                $"input[name='Alternativas[{indice}].Texto']"
            ).FillAsync(alternativas[indice]);

            if (await page.Locator(
                $"input[name='Alternativas[{indice}].Correta']"
            ).IsCheckedAsync())
            {
                await page.Locator(
                    $"input[name='Alternativas[{indice}].Correta']"
