...
public sealed class PedidoContaE2ETests : E2ETestsBase
{
    ...
    public async Task CT_PED_017_DeveAtualizarValorTotal_AoAdicionar_Alterar_ERemoverPedidos()
    {
        ...
        await _produtoFormPage.PreencherFormularioAsync(
            "Cerveja Long Neck",
            "10.00"
        );
        ...
        await Expect(
            ...
            _contaDetalhesPage.Url(contaId)
        );

        await Expect(
            _contaDetalhesPage.ValorTotal
        ).ToContainTextAsync(
            new Regex(@"20[.,]00")
        );

        // Alterar: quantidade 2 -> 5, total deve virar R$50,00.
        await _contaDetalhesPage.AlterarQuantidadeAsync(
            "Cerveja Long Neck",
            5
        );
        ...
        await _contaDetalhesPage.RemoverAsync(
            "Cerveja Long Neck"
        );
    ...
    private async Task<Guid> AbrirContaAsync()
    {
        ...
        // 5. Captura o ID da conta a partir da URL.
