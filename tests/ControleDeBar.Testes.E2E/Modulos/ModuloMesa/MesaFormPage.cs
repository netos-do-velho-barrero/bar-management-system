...
public class MesaFormPage(IPage page, string urlBase)
{
    ...
    public ILocator CampoNumero =>
        _page.Locator("input[name='Numero']");

    public ILocator CampoQuantidadeLugares =>
        _page.Locator("input[name='QuantidadeLugares']");
        ...
        int quantidadeLugares)
    {
        await CampoNumero.FillAsync(
            numero > 0 ? numero.ToString() : string.Empty
        );

        await CampoQuantidadeLugares.FillAsync(
            quantidadeLugares.ToString()
        );
    }
